...
void TrtGptModelInflightBatching::createDecoder(std::optional<executor::DecodingMode> const& decodingModeOpt)
{
    ...
    if (mWorldConfig.isLastPipelineParallelRank())
    {
        ...
        {
            mDecoders.push_back(
                std::make_shared<runtime::GptDecoderBatched>(
            ...
            decoder->setup(
                decodingMode, getMaxNumSequences(),
                mOperatingBeamWidth, getMaxAttentionWindow(),
    ...
    std::optional<std::vector<executor::AdditionalModelOutput>> const& additionalModelOutputs)
{
    ...
    for (SizeType32 i = 0; i < mNumMicroBatches; ++i)
    {
        ...
        mDecoderOutputBuffers.emplace_back();
        for (SizeType32 vid = 0; vid < getNumVocabs(); vid++)
        {
            mDecoderOutputBuffers[i].emplace_back(getMaxNumSequences(), mOperatingBeamWidth, getMaxSequenceLen(),
    ...
    for (SizeType32 vid = 0; vid < getNumVocabs(); vid++)
    {
        mDecoderBuffers.emplace_back(
            std::make_shared<DecoderBuffers>(
                getMaxNumSequences(), mOperatingBeamWidth, getMaxAttentionWindow(),
                mModelConfig.getMaxDecodingTokens(), mRuntime->getBufferManager(),
    ...
    if (mWorldConfig.isLastPipelineParallelRank() && !contextRequests.empty())
    {
        ...
        for (SizeType32 vocabId = 0; vocabId < getNumVocabs(); vocabId++)
        {
            auto [batchSlots, decoderRequests, samplingConfigs] = (*mGenerateRequestOptions)(mModelConfig, mWorldConfig,
                mDecodingConfig, contextRequests, mRuntime->getBufferManager(), logitsType, inputBuffers, buffers, vocabId);

            if (!decoderRequests.empty())
            {
                NVTX3_SCOPED_RANGE(decoderNewRequests);

                (*mCreateNewDecoderRequests)(batchSlots, decoderRequests, samplingConfigs, mModelConfig, *mDecoders[vocabId],
...
runtime::CudaEvent TrtGptModelInflightBatching::decoderStepAsync(ScheduledRequests const& scheduledRequests)
{
    ...
    for (SizeType32 vid = 0; vid < getNumVocabs(); vid++)
    {
        ...
        auto const genBufferId = mCtxGenFusion ? getFusedBufferId() : getGenerationBufferId();
        auto& genRuntimeBuffers = mBuffers.at(genBufferId);
        (*mHandleGenerationLogits)(genLogitsIndex, scheduledRequests.generationRequests, *mDecoderBuffers[vid], mModelConfig,
        ...
        mLogitsPostProcessorIsApplied
            = (*mLogitsPostProcessor)(scheduledRequests.contextRequests, scheduledRequests.generationRequests,
                mReplicateLogitsPostProcessor, *mDecoderBuffers[vid], mWorldConfig, *mRuntime, mLogitsPostProcessorBatched);
    ...
    bool returnLogProbs, runtime::CudaEvent decoderFinishEvent, SizeType32 vocabId)
{
    ...
    auto& decoderOutputBuffers = mDecoderOutputBuffers.at(getFusedBufferId()).at(vocabId);

    mCopyBufferManager.copy(*mDecoders[vocabId]->getDecoderState().getAllNewTokens(), *decoderOutputBuffers.newOutputTokensHost);
    mCopyBufferManager.copy(
        *mDecoders[vocabId]->getDecoderState().getJointDecodingOutput().lengths, *decoderOutputBuffers.sequenceLengthsHost);
    ...
    if (returnLogProbs)
    {
        mCopyBufferManager.copy(*mDecoders[vocabId]->getDecoderState().getCumLogProbs(), *decoderOutputBuffers.cumLogProbsHost);
        mCopyBufferManager.copy(*mDecoders[vocabId]->getDecoderState().getLogProbs(), *decoderOutputBuffers.logProbsHost);
    }

    if (mModelConfig.getSpeculativeDecodingMode().predictsDraftTokens())
    {
        // TODO: keep data on device for next iteration
        mDecoderBuffers[vocabId]->draftBuffers.nextDraftTokensDevice = mDecoders[vocabId]->getDecoderState().getNextDraftTokens();
        mCopyBufferManager.copy(
            *mDecoderBuffers[vocabId]->draftBuffers.nextDraftTokensDevice, *mDecoderBuffers[vocabId]->draftBuffers.nextDraftTokensHost);
    ...
    if (mModelConfig.getSpeculativeDecodingMode().needsKVCacheRewind())
    {
        ...
        mDecoderBuffers[vocabId]->draftBuffers.acceptedPackedPathsDevice = mDecoders[vocabId]->getDecoderState().getAcceptedPackedPaths();
    }
    ...
    bool returnLogProbs)
{
    ...
    for (SizeType32 vid = 0; vid < getNumVocabs(); vid++)
    { 
        auto& decoderOutputBuffers = mDecoderOutputBuffers.at(getFusedBufferId()).at(vid);
        auto& decoderBuffers = mDecoderBuffers.at(vid);
        if (mWorldConfig.isLastPipelineParallelRank())
        {
            if (broadcastPostDecoder())
            {
                DecoderStepAsyncSend::bcast(decoderOutputBuffers, *decoderBuffers, returnLogProbs, mOperatingBeamWidth,
                    mModelConfig.getSpeculativeDecodingMode().needsKVCacheRewind(), *mMpiCommTensorPara, 0);
            }
            if (mWorldConfig.isPipelineParallel())
            {
                auto const peerSend = 0;
                asyncHandles.emplace_back(std::make_unique<DecoderStepAsyncSend>(decoderOutputBuffers, *decoderBuffers,
        ...
        else
        {
            auto const peerRecv = mWorldConfig.isFirstPipelineParallelRank() ? mWorldConfig.getPipelineParallelism() - 1
                                                                             : mWorldConfig.getPipelineParallelRank() - 1;
            DecoderStepAsyncSend::recv(decoderOutputBuffers, *decoderBuffers, returnLogProbs, mOperatingBeamWidth,
...
void TrtGptModelInflightBatching::updateRequests(ScheduledRequests const& scheduledRequests)
{
    ...
    auto const& decoderOutputBuffers = mDecoderOutputBuffers.at(getFusedBufferId()).front();
...
void TrtGptModelInflightBatching::rewindKVCacheBlocks(SizeType32 numSequences)
{
    ...
    tensorrt_llm::runtime::kernels::invokeUpdateKVBlockArrayDraftTokenLocation(
        ...
        *decoderBuffers->draftBuffers.acceptedPackedPathsDevice, *runtimeBuffers.sequenceLengthsDevice,
...
void TrtGptModelInflightBatching::changeSpecDecMode(ScheduledRequests const& scheduledRequests)
{
    ...
    if (mModelConfig.getSpeculativeDecodingMode().isNone() && canUseLookahead && canChangeToLookahead)
    {
        ...
        for (SizeType32 vid = 0; vid < getNumVocabs(); vid++)
        {
            mDecoderOutputBuffers.at(getFusedBufferId()).at(vid).enableLookaheadDecoding(
                getMaxNumSequences(), mModelConfig.getMaxDecodingTokens()
            );
        }
        createDecoder(mDecodingConfig.getDecodingMode());
    }
        ...
        && (!canUseLookahead || numRequests > maxNumRequestForLad))
    {
        ...
        for (SizeType32 vid = 0; vid < getNumVocabs(); vid++)
        {
            mDecoderOutputBuffers.at(getFusedBufferId()).at(vid).disableLookaheadDecoding(getMaxNumSequences());
