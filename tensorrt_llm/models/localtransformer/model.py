...
class LocalTransformerDecoderLayer(Module):
...
class LocalTransformerModel(PretrainedModel):
    ...
    def __init__(self, config: PretrainedConfig):
        ...
        self.decoder_num_heads = self.config.decoder_num_heads
        decoder_num_kv_heads = None if not hasattr(
            self.config,
            "decoder_num_kv_heads") else self.config.decoder_num_kv_heads
        if decoder_num_kv_heads is None or decoder_num_kv_heads <= 0:
            decoder_num_kv_heads = self.decoder_num_heads
        self.decoder_num_kv_heads = decoder_num_kv_heads
        self.decoder_head_size = self.decoder_hidden_size // self.num_heads if self.config.decoder_head_size is None else self.config.decoder_head_size
        ...
        if self.mapping.is_first_pp_rank():
            self.in_projection = ColumnLinear(
                self.config.decoder_hidden_size,
                self.config.local_transformer_hidden_size,
                bias=False,
    ...
    def forward(self,
                local_transformer_input_ids: Tensor,
                ...
                cross_kv_reuse: Optional[Tensor] = None):
        if self.mapping.is_first_pp_rank():
            assert isinstance(local_transformer_input_ids, Tensor)
        else:
            assert isinstance(hidden_states, Tensor)

        # In PP, layer 0 has ids as inputs, all other layers have hidden_states as inputs
        if self.mapping.is_first_pp_rank():
            hidden_states = self.in_projection(local_transformer_input_ids)
        else:
        ...
        for i, (decoder_layer, past) in enumerate(
            ...
            hidden_states = decoder_layer(
                hidden_states,
                encoder_output=decoder_hidden_states,
                attention_prior_focus=attention_prior_focus,
